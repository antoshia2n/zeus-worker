...
//
// 2026-08-16 変更（2 回目）：モデルを voyage-3.5 から voyage-4-lite に変えた。
//   理由は料金。voyage-3.x 系には無料の枠が無く、voyage-4 世代には 1 アカウントにつき
//   2 億トークンぶんの無料の枠が付く。作る本数の寸法は 1024 のままで変わらない。
//   注意 1：これで 1 分あたり 3 回・1 分あたり 1 万トークンの制限が外れるわけではない。
//     この制限は支払い方法が未登録のアカウントに掛かるもので、モデルごとではない。
//     上の間隔待ちとやり直しはそのまま残す。
//   注意 2：モデルを変えると、作られるベクトルの土俵が変わる。voyage-3.5 で作った
//     古いベクトルと混ぜると、近さの比べ方が狂う。毎晩の全件の入れ替えで全部作り直る
//     あいだは問題にならないが、変わったぶんだけ入れ直す作りへ移すときは、
//     その前に一度すべて作り直しておく必要がある。
